...
@section('breadcrumb')
    {{ Breadcrumbs::render('season_course_list',$id) }}
...
@endsection
@section('button')
    <a class="btn btn-primary" href="/admin/season_create">افزودن فصل دوره</a>
@endsection
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#tblData').DataTable({
                ...
                "ajax":{
                    url:"/api/admin/course/season_course",
                    data:{
                        course_id:{{ $id }}
                    },
                ...
                "columns":[
                    ...
                    {
                        "render": function (data, type, JsonResultRow, meta) {
                            return '<a href="'+JsonResultRow.video_link+'">دانلود</a>';
                        },
                    },
                    {
                        "render": function (data, type, JsonResultRow, meta) {
                            if(JsonResultRow.status_free == false){
                                var state = "<div class='badge badge-info'>پیش نمایش ندارد</div>"
                            }else{
                                var state = "<div class='badge badge-info'>پیش نمایش دارد</div>"
                    ...
                    {data:'Priority'},
                    {
                        ...
                        "render": function ( data, type, full, meta ) {
                            return "<button onclick='delete_course("+full.id+")' style='background: transparent;border: none'><span class='action-edit'><i class='feather icon-trash'></i></span></button>";
                        }
                    }
                ],
        ...
        function delete_course(id){
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url:"/api/admin/course/season_course/"+id,
                        type:"post",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader ("Authorization", "Bearer " + token);
                        },
